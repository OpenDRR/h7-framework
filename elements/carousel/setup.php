...
class Carousel {
	...
	public function open ( $element ) {
		...
		if ( isset ( $GLOBALS['elements']['types'][$type]['carousel'] ) ) {
			...
			echo '<div id="' . get_current_element_ID() . '-swiper-container" class="swiper-container carousel ';
...
// SETUP
/*
function carousel_setup ( $block_ID ) {
  ...
  $this->classes['controls'][] = 'has-navigation';
    ...
    $this->settings['navigation'] = array (
      'prevEl' => '#' . $block_ID . '-prev',
      'nextEl' => '#' . $block_ID . '-next'
    );
...
function carousel_output ( $element, $action ) { // $action, $type, $render = null ) {
  ...
  if ( isset ( $GLOBALS['elements']['types'][$type]['carousel'] ) ) {
    ...
    $carousel = $GLOBALS['elements']['types'][$type]['carousel'];
	...
	remove_action ( 'after_element_wrapper_open', 'carousel_output', 20, 2 );
